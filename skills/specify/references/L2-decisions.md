...
**Output**: Append `## Decisions`, `## Constraints`, `## Known Gaps` sections to spec.md
...
```
Example:
Q: "What happens after game over?"
A: "A restart button" → no discriminator → provisional
   → Core Behavior (high-impact) → follow-up:
   "Does restart reset the rings/score, or restart from the beginning of the stage?"
A2: "Full reset, back to stage 1" → discriminator (explicit behavior) → resolved
```
